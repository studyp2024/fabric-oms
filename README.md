...
- **OS**: Ubuntu 22.04 LTS (个人部署使用的linux系统版本)
- **Java**: JDK 8
- **Node.js**: 18.0+ (必需，用于 Vite 构建)
- **Docker**: 最新版
- **Hyperledger Fabric**: 2.5.14 (集成在本项目中)
- **MySQL**: 8.0
...
- **内嵌 WebSSH 终端**: 支持直接在浏览器中通过 WebSSH 连接目标服务器进行操作，彻底隐藏服务器真实密码，提升安全性。
- **精准身份追踪**: 即使多名运维人员共用目标服务器的同一 SSH 账号，系统也能通过底层环境变量注入技术，精准区分并审计出每条命令对应的真实系统用户名。
...
- **管理员 (Admin)**:
    - 拥有系统最高权限。
    - 添加/管理运维人员和审计员。
    - 添加/管理受控服务器，并将服务器分配给一个或多个指定的运维人员 (支持一对多分配)。
    - 查看所有服务器的密码，并通过 WebSSH 登录任意服务器。
    - 查看全局审计日志。
- **运维人员 (Ops)**:
    - 仅能查看分配给自己的服务器。
    - 无法查看服务器真实密码，只能通过系统提供的 WebSSH 按钮一键安全登录。
    - 无法查看全局审计日志和管理用户。
- **审计员 (Auditor)**:
    - 专职审计角色。
    - 只能查看全局审计日志，无法进行服务器连接和用户管理等其他任何操作。
...
5.  **运行前端**: `npm install` && `npm run dev` (开发) 或 Nginx 部署 (生产)。
